...
public class LogInManager : MonoBehaviour
{
    ...
    public GameObject rejectionCanvas;
    ...
    public void LoginInAgain()
    {
        if (usernameInput.text == "Username" && passwordInput.text == "Password")
        {
            ...
            {
                Debug.Log("you're admitted to Village house");
                loginCanvas.SetActive(false);
                rejectionCanvas.SetActive(false);
                // DoTween DoMove(Village)
            ...
            {
                Debug.Log("Welcome to Medieval City House");
                loginCanvas.SetActive(false);
                rejectionCanvas.SetActive(false);
                // DoTween DoMove(City)
                cityCamera.transform.DOMove(cityTarget.position, timeDelay);
                cityCamera.transform.DORotate(new Vector3(0, 180, 0), timeDelay);
            }
            else
            {
                loginCanvas.SetActive(false);
